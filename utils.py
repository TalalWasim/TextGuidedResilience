...
class CLIPGuidedClassifier(nn.Module):
    def __init__(self,
        ...
        self.backbone = torchvision.models.get_model(backbone_type, weights=weights, num_classes=embed_size)
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        
        self.head = nn.Linear(embed_size, num_classes, bias=False)

        if CLIP_text_path is None:
            print('NO TEXT FEATURES: RANDOM INITIALIZATION')
        else:
            print('TEXT FEATRUES FOUND: ROBUST INITIALIZATION')
            text_features = torch.load(CLIP_text_path)
            text_features = text_features / text_features.norm(dim=1, keepdim=True)
            sd = {'weight': text_features}
    ...
    def forward(self, x):
        ...
        logits = logit_scale * self.head(image_features)
